...
import { VoteProgressBarComponent } from '@features/polls/presentation/dumb/vote-progress-bar.component';
import { AuthStore } from '@store/auth/auth.store';
import { AuthModalComponent } from '@features/polls/presentation/dumb/auth-modal.component';
...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto py-8 md:py-12 font-sans box-border relative">
          ...
          <div class="animate-fade-in">
            ...
            >
              <span class="inline-flex items-center gap-1.5">
                <i class="pi pi-calendar"></i> Créé le {{ poll.createdAt }}
              </span>
              <span class="inline-flex items-center gap-1.5">
                <i class="pi pi-user"></i> par &#64;{{ poll.createdBy }}
              </span>
              <span class="inline-flex items-center gap-1.5">
                <i class="pi pi-clock"></i>
                {{
                  poll.status === 'closed'
                    ? 'Clos le ' + poll.expiresAt
                    : 'Expire le ' + poll.expiresAt
                }}
              </span>
            </div>
            ...
            @if (facade.showResults()) {
              <div class="flex flex-col gap-5">
                ...
                <div class="mt-2 flex items-center gap-1.5 text-xs text-muted-color font-medium">
                  <i class="pi pi-users"></i> {{ poll.totalVotes }} votes au total
                </div>
              ...
              <div>
                ...
                >
                  ...
                  <span
                    >Votre vote sera <span class="font-bold">définitif</span> et ne pourra pas être
                    modifié.</span
                  >
                </div>
                ...
                @if (isAuth && !session?.emailVerified) {
                  <div
                    class="mb-4 rounded-xl border border-orange-200 bg-orange-50/40 p-3 text-xs text-orange-600 flex items-center gap-3 font-medium box-border"
                  >
                ...
                <div
                  ...
                  [ngClass]="
                    !isAuth || !session?.emailVerified ? 'opacity-60 pointer-events-none' : ''
                  "
                >
                  @for (option of poll.options; track option.id) {
                    <label
                    ...
                    >
                      <input
                        type="radio"
                        name="vote"
                        [disabled]="!isAuth || !session?.emailVerified"
                        [value]="option.id"
                ...
                <div class="mt-8 flex items-center gap-2">
                  ...
                  @if (isAuth) {
                    <p-button
                      [label]="!session?.emailVerified ? 'Email non vérifié' : 'Soumettre mon vote'"
                      severity="primary"
                      size="small"
                      [icon]="session?.emailVerified ? 'pi pi-check' : ''"
                      styleClass="font-bold px-4 shadow-sm"
                      [disabled]="!facade.selectedOption() || !session?.emailVerified"
                      (click)="submitVote()"
                    />
                  } @else {
                    <p-button
      ...
      <app-auth-modal [(visible)]="showAuthModal"></app-auth-modal>
    </div>
  `,
})
export class PollDetailViewComponent {
  ...
  protected submitVote(): void {
    const selected = this.facade.selectedOption();
    if (!selected) return;
